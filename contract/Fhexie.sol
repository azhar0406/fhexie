...
contract Fhexie is ERC1155, Ownable, ERC1155Supply {
  ...
  euint8 public MAX_ATTACK_DEFEND_STRENGTH = FHE.asEuint8(10) ;
  ...
  function initialize() private {
    gameTokens.push(GameToken("", 0,FHE.asEuint8(0), FHE.asEuint8(0)));
  ...
  function _createRandomNum() internal view returns (euint8 ) {
    euint8 randomNum =  FHE.asEuint8(getRandomNumber(uint8(block.number)));
    return randomNum;
  }
  ...
  function _createGameToken(string memory _name) internal returns (GameToken memory) {
    ...
    euint8 randDefenseStrength = FHE.sub(MAX_ATTACK_DEFEND_STRENGTH , randAttackStrength);
    
    uint8 randId = FHE.decrypt(_createRandomNum());
    randId = randId % 6;
    if (randId == 0) {
      randId++;
    }
    ...
    totalSupply++;
    uint8 rnd = FHE.decrypt(randAttackStrength);
    uint8 defense = FHE.decrypt(randAttackStrength);
  ...
  function attackOrDefendChoice(inEuint8 calldata choice, string memory _battleName) external {
    Battle memory _battle = getBattle(_battleName);

   uint8 _choice = FHE.decrypt(FHE.asEuint8(choice));
  ...
  function _resolveBattle(Battle memory _battle) internal {
    ...
    if (p1.move == 1 && p2.move == 1) {
      if (FHE.decrypt(p1.attack) >= (p2.health)) {
        _endBattle(_battle.players[0], _battle);
      } else if (FHE.decrypt(p2.attack) >= p1.health) {
        _endBattle(_battle.players[1], _battle);
      } else {
        players[p1.index].playerHealth -= FHE.decrypt(p2.attack);
        players[p2.index].playerHealth -= FHE.decrypt(p1.attack);
      ...
      uint256 PHAD = p2.health + FHE.decrypt(p2.defense);
      if (FHE.decrypt(p1.attack) >= PHAD) {
        _endBattle(_battle.players[0], _battle);
      } else {
        uint256 healthAfterAttack;
        
        if(FHE.decrypt(p2.defense) > FHE.decrypt(p1.attack)) {
          healthAfterAttack = p2.health;
        } else {
          healthAfterAttack = PHAD - FHE.decrypt(p1.attack);
      ...
      uint256 PHAD = p1.health + FHE.decrypt(p1.defense);
      if (FHE.decrypt(p2.attack) >= PHAD) {
        _endBattle(_battle.players[1], _battle);
      } else {
        uint256 healthAfterAttack;
        
        if(FHE.decrypt(p1.defense) > FHE.decrypt(p2.attack)) {
          healthAfterAttack = p1.health;
        } else {
          healthAfterAttack = PHAD - FHE.decrypt(p2.attack);
